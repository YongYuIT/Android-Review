...
(JNIEnv *, jclass){
	...
	if (1 == 1){
		...
		memcpy(input_data, input.data(), input_lg);
		uint8_t* iv = (uint8_t*)malloc(size);
		memset(iv, 0, size);
		RAND_pseudo_bytes(iv, size);
		uint8_t* en_result = EncryptionTools::do_en_de(size, input_data, key, AES_ENCRYPT);
		uint8_t* de_result = EncryptionTools::do_en_de(size, en_result, key, AES_DECRYPT);
		__android_log_print(ANDROID_LOG_INFO, "yuyong", "ttttttttttttttttttttestt --> %s", de_result);
	}
}